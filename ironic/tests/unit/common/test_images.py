...
class IronicImagesTestCase(base.TestCase):
    ...
    @mock.patch.object(image_format_inspector, 'detect_file_format',
                       autospec=True)
    def test_converted_size(self, img_info_mock):
        info = self.FakeImgInfo()
        info.virtual_size = 1
        img_info_mock.return_value = info
        size = images.converted_size('path')
        img_info_mock.assert_called_once_with('path')
        self.assertEqual(1, size)

    @mock.patch.object(image_format_inspector, 'detect_file_format',
                       autospec=True)
    def test_get_image_mb(self, image_info_mock):
        info = self.FakeImgInfo()
        info.disk_size = 2
        info.virtual_size = 717401213
        image_info_mock.return_value = info
        size = images.get_image_mb('path')
        image_info_mock.assert_called_once_with('path')
        self.assertEqual(685, size)
